...
class HealthDashboard {
    ...
    updateSyncStatus(lastUpdated) {
        ...
        if (statusEl && lastUpdated) {
            ...
            if (diffSeconds < 60) {
                ...
                statusEl.textContent = `Synced ${diffMinutes}m ago`;
                statusEl.className = 'sync-status';
            } else if (diffMinutes < 60) {
                statusEl.textContent = `Last sync: ${diffMinutes}m ago`;
                statusEl.className = 'sync-status waiting';
            } else {
                const hours = Math.floor(diffMinutes / 60);
                statusEl.textContent = `Last sync: ${hours}h ago`;
                statusEl.className = 'sync-status waiting';
            }
        }
    }
    ...
    updateActivityRings(rings) {
        ...
        const moveProgress = Math.min((move / moveGoal), 2.0); // Allow up to 200%
        
        const exercise = rings.exerciseMinutes || 0;
        const exerciseGoal = rings.exerciseGoal || 30;
        const exerciseProgress = Math.min((exercise / exerciseGoal), 2.0);
        
        const stand = rings.standHours || 0;
        const standGoal = rings.standGoal || 12;
        const standProgress = Math.min((stand / standGoal), 2.0);
        
        // Update SVG ring animations
        this.updateSVGRing('ring-move', moveProgress, 565.48);
        this.updateSVGRing('ring-exercise', exerciseProgress, 439.82);
        this.updateSVGRing('ring-stand', standProgress, 314.16);
        
        // Update ring stats
        this.updateRingStat('move', move, moveGoal, moveProgress * 100, 'CAL');
        this.updateRingStat('exercise', exercise, exerciseGoal, exerciseProgress * 100, 'MIN');
        this.updateRingStat('stand', stand, standGoal, standProgress * 100, 'HRS');
    }
    
    /**
     * Update SVG ring with accurate progress
     * @param {string} ringId - The ID of the ring element
     * @param {number} progress - Progress as decimal (0.0 to 2.0 for up to 200%)
     * @param {number} circumference - The circumference of the circle
     */
    updateSVGRing(ringId, progress, circumference) {
        const ring = document.getElementById(ringId);
        if (ring) {
            // Calculate stroke-dashoffset
            // At 0% progress, offset = circumference (nothing visible)
            // At 100% progress, offset = 0 (full circle)
            // At 200% progress, we wrap around (show full circle + extra)
            const cappedProgress = Math.min(progress, 1.0); // Cap visual at 100% for single circle
            const offset = circumference * (1 - cappedProgress);
    ...
    showNoDataState() {
        const statusEl = document.getElementById('syncStatus');
        if (statusEl) {
            statusEl.textContent = 'Waiting for Apple Watch...';
            statusEl.className = 'sync-status waiting';
        }
    }
