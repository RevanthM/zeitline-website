...
class EventsManager {
    ...
    setupEventListeners() {
        ...
        // Drop zone - make the entire tasks panel droppable
        ...
        const handleDragOver = (e) => {
            ...
            if (tasksContent) tasksContent.classList.add('drag-over');
            if (tasksPanel) tasksPanel.classList.add('drag-over');
        };
        
        // Helper to handle drag leave
        const handleDragLeave = (e) => {
            // Only remove classes if we're actually leaving the panel
            // Check if the related target is still within the panel
            const relatedTarget = e.relatedTarget;
            if (tasksPanel && tasksPanel.contains(relatedTarget)) {
                return; // Still inside the panel, don't remove classes
            }
            if (dropZone) dropZone.classList.remove('active');
            if (tasksContent) tasksContent.classList.remove('drag-over');
            if (tasksPanel) tasksPanel.classList.remove('drag-over');
        };
        ...
        // Attach listeners to the small drop zone
        if (dropZone) {
        ...
        // Attach listeners to the entire tasks content area
        if (tasksContent) {
            tasksContent.addEventListener('dragover', handleDragOver);
            tasksContent.addEventListener('dragleave', handleDragLeave);
            tasksContent.addEventListener('drop', handleDrop);
        }
        
        // Attach listeners to the entire tasks panel (header and all)
        if (tasksPanel) {
            tasksPanel.addEventListener('dragover', handleDragOver);
            tasksPanel.addEventListener('dragleave', handleDragLeave);
            tasksPanel.addEventListener('drop', handleDrop);
        }
